...
async function fetchColorData() {
    ...
    setTimeout(async () => {
      try {
        ...
        if (!response.ok) throw new Error(`HTTP Error: ${response.status}`); //* შეამოწმებს სწორად მიიღო მოანებები თუ არა მაშინ დაბეჭდავს ერორის სტატუს კოდს
        ...
        //*1.ელმენეტების შექმნა/ჩამატება ლისტში, 2.background ფერის მინიჭება data წამორებული ფერებით, 3. მონაცენების დაბეჭდვა
        data.forEach(item => {
